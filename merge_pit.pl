...
$Branch = shift || "nos6.0.0_pit_a";
$MyBranch = shift || `cleartool catcs | grep -e "^mkbranch" | tail -1 | cut -f2 -d" "`;
$no_zero_merge = shift;
my $view_prefix = "fabos_p_Nightly_";
#my $view_prefix = "swrel_"; # was "fabos_p_Nightly_"
chomp($MyBranch);

print STDERR "Merge to $Branch as of $Date to $MyBranch\n";
#print `refresh_view -rci`;
my $from_view = $view_prefix . $Branch . "_" . $Date;
...
my @newcs = ();
my $changed_cs = 0;
my $fver = "-ftag $view_prefix$Branch" . "_" . $Date;;
unless ($Date =~ /k/i) {
...
while ($file = <FILELIST>) {
	...
	if ($no_zero_merge) {
		my $ccfile = "";
		if ($Type eq "f") {
			$ccfile = `cleartool ls -s $file`;
		} else {
			$ccfile = `cleartool ls -dir -s $file`;
		}
		chomp($ccfile);
		...
		if ($skip_zero == 0) {
			if ($Type eq "f") { push(@files_to_merge, $file); }
			else              { push(@dirs_to_merge, $file); }
		}
	}
	if ($skip_zero == 0) {
